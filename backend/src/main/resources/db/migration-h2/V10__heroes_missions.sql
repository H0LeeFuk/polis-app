-- (H2-compatible) Two heroes per account (Leo + Celine) + the mission system.
...
-- copy WITHOUT id so the identity sequence advances correctly (nothing FKs hero ids)
...
create table missions (
  ...
  objective_params         json not null default '{}',
...
-- H2 needs FORMAT JSON to store a JSON object from a string literal (else it's a JSON string).
insert into missions (chain, order_index, title, description, objective_type, objective_target, rewards, unlocks_hero_key) values
 ('STARTER',1,'First Foundations','Construct 2 buildings in your city.','BUILD_BUILDING',2,'{"wood":150,"stone":150}' format json,null),
 ('STARTER',2,'A Growing Polis','Raise any building to level 3.','UPGRADE_BUILDING_LEVEL',3,'{"wood":200,"stone":200}' format json,null),
 ('STARTER',3,'Raise an Army','Train 10 troops.','TRAIN_TROOPS',10,'{"silver":200}' format json,null),
 ('STARTER',4,'Blood the Blade','Defeat a bandit camp.','ATTACK_BANDIT_CAMP',1,'{"wood":200,"stone":200,"heroXp":50}' format json,null),
 ('STARTER',5,'Knowledge is Power','Raise your Academy to level 2.','REACH_ACADEMY_LEVEL',2,'{"silver":250}' format json,null),
 ('STARTER',6,'The Spoils of War','Launch an attack on another player.','ATTACK_PLAYER',1,'{"silver":300,"heroXp":50}' format json,null),
 ('STARTER',7,'Expand the Realm','Found your second city.','FOUND_CITY',1,'{"wood":300,"stone":300,"silver":200}' format json,null),
 ('STARTER',8,'A New Ally Arrives','Complete the starter trials to recruit Celine the Fairy.','CHAIN_COMPLETE',1,'{"wood":500,"stone":500,"silver":300}' format json,'CELINE');
