-- PART: Founding Cities & Race Selection.
-- Races replace the unused god slot, each city/unit gains a race, and the four races get
-- distinct unit rosters. The existing base roster becomes the Human roster.
...
-- a hero parks at a slot on arrival; arrived_at marks the pending-founding wait.
alter table movements   add column arrived_at timestamptz;

-- distinct per-race rosters (Humans already seeded in V5; add a Human warship + 3 new rosters).
insert into unit_types
  ...
  -- HUMANS: balanced fleet flagship
  ('TRIREME',        'SHARP',     90,  60,  60,  60, 20, 200,  6, 'SEA',  'HARBOR',   45, 300, 120, 180, null,       'HUMANS'),

  -- GIANTS: brutal, tanky, slow, population-hungry
  ...
  -- FAIRIES: blazing speed and economy, fragile in defence
  ...
  -- NEWTS: dominant navy and quick crossings, modest on land
